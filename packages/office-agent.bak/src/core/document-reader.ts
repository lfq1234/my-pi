...
import { detectSourceType, extractDocumentText } from "../ingestion/office-parser.ts";
...
export async function extractDocumentStructure(filePath: string): Promise<OfficeDocumentSummary> {
	...
	const warnings =
		document.metadata.warningCount && Number(document.metadata.warningCount) > 0
...
async function listSupportedFiles(dirPath: string): Promise<string[]> {
	...
	for (const entry of entries) {
		...
		const supported = [".md", ".txt", ".html", ".htm", ".csv", ".pdf", ".docx", ".xlsx", ".pptx"];
...
async function extractDocxText(buffer: Buffer): Promise<string> {
	...
	const xmlCandidates = Object.keys(zip.files).filter(
		(name) => name.endsWith(".xml") && /(^|\/)(word|document)\//i.test(name),
	);
...
async function extractWpsCompatibleText(buffer: Buffer, kind: "doc" | "sheet" | "slide"): Promise<string> {
	...
	for (const name of xmlFiles) {
		...
		const matches = Array.from(
			xml.matchAll(
				/<(?:w|a|c|t|v|p|s|text|Text|para|Paragraph|item)[^>]*>([\s\S]*?)<\/(?:w|a|c|t|v|p|s|text|Text|para|Paragraph|item)>/gi,
			),
		);
...
function buildStructuredSummary(content: string, kind: OfficeFileKind, warnings: string[]): OfficeDocumentSummary {
	...
	const sections: OfficeReportSection[] = [
		...
		},
		{
			...
			bullets:
				warnings.length > 0 ? warnings : ["Content was extracted and normalized for downstream report generation."],
		},
	];
