...
function candidates(): string[] {
  ...
  if (process.platform === "win32") {
    ...
    list.push("/usr/bin/soffice", "/usr/local/bin/soffice", "/usr/bin/libreoffice", "/opt/libreoffice/program/soffice");
  }
  return list;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      ...
      if (code === 0 && existsSync(out)) {
        ...
        reject(new Error(`LibreOffice 转换失败 (exit ${code ?? "?"}): ${src} → ${targetExt}${stderr ? `\n${stderr}` : ""}`));
      }
    });
  });
}
