...
import { buildPreviewPage, createGeneratedReport } from "../tools/html-generator.ts";
import { createPosterBrief } from "../tools/poster-brief.ts";
import { ensureDir, makeId, normalizeOutputDirectory, nowIso, readTextFile, writeTextFile } from "../utils.ts";
import type {
	OfficeAgentOptions,
	OfficeAgentRunInput,
	OfficeAgentRunResult,
	OfficeDocument,
	OfficeFileKind,
} from "./types.ts";
...
export class OfficeAgent {
	...
	async run(input: OfficeAgentRunInput = {}): Promise<OfficeAgentRunResult> {
		...
		const posterBrief = createPosterBrief(
			baseTitle,
			input.audience ?? "general audience",
			input.style ?? "modern business",
			summary,
		);
		...
		return {
			...
			seedance: {
				...
				message:
					seedance.status === "generated"
						? "Seedance generation completed."
						: "Seedance request stayed in draft mode because no API key was configured.",
