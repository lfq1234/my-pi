...
function formatPrompt(task: OfficeMultiAgentTask, previousOutput?: string): string {
  ...
  const transformed = prompt.includes("{previous}")
...
export async function runOfficeMultiAgentWorkflow(input: OfficeMultiAgentWorkflowInput): Promise<OfficeMultiAgentWorkflowResult> {
