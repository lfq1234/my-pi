...
import { createExcelWorkbook, createPowerPointDeck, createWordDocument } from "../src/index.ts";

test("createWordDocument builds a valid docx package", async () => {
	const bytes = await createWordDocument({
		title: "Quarterly Review",
		paragraphs: ["Revenue up 18%", "Retention improved."],
	});
